...
public class CategoryAxis extends Axis implements Cloneable, Serializable {
    ...
    protected TextBlock createLabel(Comparable category, float width,
    	...
    	TextBlock label = createTextBlock(category.toString(),
    ...
    public static TextBlock createTextBlock(String text, Font font,
        ...
        while (current < length && lines < maxLines) {
            ...
            //added
            // If we're on the last line add as much as we can so we don't wrap
            if(lines == maxLines - 1)
            { 
            // Keep adding words until we go past the maxWidth (and keep it!)
            while(next != BreakIterator.DONE && measurer.getStringWidth(text, current, next) < maxWidth)
            	next = iterator.next();             
            	if(next == BreakIterator.DONE)
            		next = text.length();
            }
       ...
       /* if (current < length) {
        ...
        int offset = 4;
        while (offset > oldStr.length())
        	offset--;
        while(offset < oldStr.length() && measurer.getStringWidth(oldStr.substring(0, offset), 0, offset) < maxWidth)
        	++offset;
