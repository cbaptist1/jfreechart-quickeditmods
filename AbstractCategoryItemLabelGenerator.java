...
public abstract class AbstractCategoryItemLabelGenerator
     ...
     * {2} = value, {3} = value as a percentage of the column total. {4} value as a percentage of the row total
     */
    ...
    protected Object[] createItemArray(CategoryDataset dataset,
        ...
        if (value != null) {
        	...
        	boolean useRow = true;
        	if (useCol) {
        		double colTotal = DataUtilities.calculateColumnTotal(dataset, column);
        		double colPercent = value.doubleValue() / colTotal;
        		result[3] = this.percentFormat.format(colPercent);
        		//System.out.format("value %f total %f percent %f formatted %s\n", value, total, percent, result[3]);
        	}
        	if (useRow) {
        		//double rowTotal = DataUtilities.calculateRowTotal(dataset, row);
        		result[4] = this.percentFormat.format(value.doubleValue() / DataUtilities.calculateRowTotal(dataset, row));
        	}
