...
public class ChartPanel extends JPanel implements ChartChangeListener,
    ...
    public static final String SAVE_AS_PDF_COMMAND = "SAVE_AS_PDF";
    
    /** Action command to save all charts as PNG. */
    public static final String SAVE_ALL_AS_PNG_COMMAND = "SAVE_ALL_AS_PNG";
    
    /** Action command to save as SVG. */
    public static final String SAVE_ALL_AS_SVG_COMMAND = "SAVE_ALL_AS_SVG";
    
    /** Action command to save as PDF. */
    public static final String SAVE_ALL_AS_PDF_COMMAND = "SAVE_ALL_AS_PDF";
    ...
    public static final String ZOOM_TO_LIMITS_COMMAND = "ZOOM_TO_LIMITS";
    public static final String ZOOM_TO_DATA_COMMAND = "ZOOM_TO_DATA";
    ...
    private Range naturalXRange = null;
    private Range naturalYRange = null;
    
    private Double lowXLimit = null;
    private Double highXLimit = null;
    
    private Double lowYLimit = null;
    private Double highYLimit = null;
    ...
    private boolean includeScaleToLimits = false;
    
    private boolean includeSaveAll = true;
    ...
    /** Menu item for zooming domain to limits */
    private JMenuItem zoomToLimitsMenuItem;
    
    private JMenuItem zoomToDataMenuItem;
    ...
    public void actionPerformed(ActionEvent event) {
       ...
       /* else if (command.equals(SAVE_AS_PDF_COMMAND)) {
        ...
        }else if (command.equalsIgnoreCase(ZOOM_TO_LIMITS_COMMAND)){
        	zoomToLimits();
        }else if (command.equalsIgnoreCase(ZOOM_TO_DATA_COMMAND)){
        	zoomToData();
        }
    ...
    private void zoomOneAxis(Axis axis, Double lowLimit, Double highLimit){
    	if (axis instanceof NumberAxis){
			NumberAxis numericAxis = (NumberAxis)axis;
			Range range = numericAxis.getRange();
			
			double lower = range.getLowerBound();
			double upper = range.getUpperBound();
			if (lowLimit != null){
				lower = lowLimit.doubleValue();
			}
			if (highLimit != null){
				upper = highLimit.doubleValue();
			}
    ...
    public void zoomToLimits(){
    	...
    	if (plot instanceof XYPlot){
    		...
    		rangeAxis = cPlot.getRangeAxis();
    	}
    	if (this.naturalXRange == null && this.naturalYRange == null){
    		if (domainAxis != null){
    			domainAxis.setAutoRange(true);
    			this.naturalXRange = domainAxis.getRange();
    		}
    		if (rangeAxis != null){
    			rangeAxis.setAutoRange(true);
    			this.naturalYRange = rangeAxis.getRange();
    		}
    	}
    	if(rangeAxis != null)
    		zoomOneAxis(rangeAxis, this.lowYLimit, this.highYLimit);
    		//xyPlot.zoomRangeAxes(1.0, this.info.getPlotInfo(), 
    			//	translateScreenToJava2D(new Point(0, 0)));
    	if (domainAxis != null)
    		zoomOneAxis(domainAxis, this.lowXLimit, this.highXLimit);
    		//xyPlot.zoomDomainAxes(1.0, this.info.getPlotInfo(), 
    			//	translateScreenToJava2D(new Point(0, 0)));
    ...
    public void zoomToData(){
    	...
    	if (plot instanceof XYPlot){
    		XYPlot xyPlot = (XYPlot)plot;
    		if (this.naturalXRange != null)
    			xyPlot.getDomainAxis().setRange(this.naturalXRange, false, true);
    		if (this.naturalYRange != null)
    			xyPlot.getRangeAxis().setRange(this.naturalYRange, false, true);
    	}else if (plot instanceof CategoryPlot){
    		if (this.naturalYRange != null)
    			((CategoryPlot)plot).getRangeAxis().setRange(this.naturalYRange, false, true);
    	}
    }
    /**
    ...
    protected JPopupMenu createPopupMenu(boolean properties,
        ...
        if (save) {
            ...
            //save all charts
            if (this.includeSaveAll){
            	JMenu saveAllSubMenu = new JMenu("Save all as");
            	JMenuItem pngAllItem = new JMenuItem("PNG...");
            	pngAllItem.setActionCommand(SAVE_ALL_AS_PNG_COMMAND);
            	pngAllItem.addActionListener(this);
            	saveAllSubMenu.add(pngAllItem);
            	//todo
            	/* if (createSVGGraphics2D(10, 10) != null) {
                JMenuItem svgAllItem = new JMenuItem("SVG...");
                svgAllItem.setActionCommand(SAVE_ALL_AS_SVG_COMMAND);
                svgAllItem.addActionListener(this);
                saveAllSubMenu.add(svgAllItem);                
            }*/

            	//if (isOrsonPDFAvailable()) {
            	JMenuItem pdfAllItem = new JMenuItem("PDF...");
            	pdfAllItem.setActionCommand(SAVE_ALL_AS_PDF_COMMAND);
            	pdfAllItem.addActionListener(this);
            	saveAllSubMenu.add(pdfAllItem);
            	//}
            	result.add(saveAllSubMenu);
            }
            separator = true;
        }
        ...
        if (zoom) {
            ...
            if (this.includeScaleToLimits){
            	this.zoomToLimitsMenuItem = new JMenuItem("Zoom To Limits");
            	this.zoomToLimitsMenuItem.setActionCommand(ZOOM_TO_LIMITS_COMMAND);
            	this.zoomToLimitsMenuItem.addActionListener(this);
            	result.add(this.zoomToLimitsMenuItem);
            	
            	this.zoomToDataMenuItem = new JMenuItem("Zoom to Data");
            	this.zoomToDataMenuItem.setActionCommand(ZOOM_TO_DATA_COMMAND);
            	this.zoomToDataMenuItem.addActionListener(this);;
            	result.add(this.zoomToDataMenuItem);
            }
    ...
    public void setIncludeSaveAll(boolean value){
    	if (value != this.includeSaveAll){
    		this.includeSaveAll = value;
    		this.popup = createPopupMenu(true, true, true, true, true);
    	}
    }
    public void setIncludeScaleToLimits(boolean value){
    	this.includeScaleToLimits = value;
    	this.popup = createPopupMenu(true, true, true, true, true);
    }

    public void setLowXLimit(Double value){
    	this.lowXLimit = value;
    }
    
    public void setHighXLimit(Double value){
    	this.highXLimit = value;
    }
    
    public void setLowYLimit(Double value){
    	this.lowYLimit = value;
    }
    
    public void setHighYLimit(Double value){
    	this.highYLimit = value;
    }
}
